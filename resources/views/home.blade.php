        ...
        <div class="container mx-auto px-4 max-w-6xl">
            ...
            <div class="flex flex-wrap justify-center gap-6">
                ...
                <div class="w-full md:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)] flex flex-col items-center text-center bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 hover:shadow-[0_8px_30px_rgb(46,139,87,0.08)] transition-all duration-300 transform hover:-translate-y-1" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    @if($adv->description)
                        <p class="text-base text-gray-500 leading-relaxed text-justify mb-4">{{ $adv->description }}</p>
                    @endif
                    @if($adv->points && count($adv->points) > 0)
                        <ul class="text-left space-y-3 w-full">
                            @foreach($adv->points as $pointItem)
                                @if(!empty($pointItem['point']))
                                <li class="flex items-start text-gray-600 text-sm md:text-base text-justify">
                                    <svg class="w-5 h-5 text-green-500 mr-2 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                    <span>{{ $pointItem['point'] }}</span>
                                </li>
                                @endif
                            @endforeach
                        </ul>
